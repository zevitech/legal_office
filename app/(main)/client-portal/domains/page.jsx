...
export default async function DomainsPage({searchParams}){
  ...
  const data=demo?null:await getClientPortalData(user.uid);const clientCase=demo?.case||data?.cases?.[0]||null;
  return <PortalServiceFunnel mode="domains" user={user} clientCase={clientCase} preview={Boolean(demo)} previewKey={previewKey}/>;
}

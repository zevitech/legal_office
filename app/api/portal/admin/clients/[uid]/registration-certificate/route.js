...
export async function POST(request, { params }) {
  ...
  try {
    ...
    if (!validation.ok) return NextResponse.json({ error: validation.error || "A valid USPTO registration certificate PDF is required." }, { status: 400 });
    await storage.bucket().file(`portal-clients/${params.uid}/certificates/${caseId}/registration-certificate.pdf`).save(validation.buffer, { resumable: false, contentType: validation.contentType, metadata: { cacheControl: "private, no-store", metadata: { uploadedBy: staff.uid } } });
    return NextResponse.json({ success: true, documentUrl: `/api/portal/documents/certificate/${params.uid}?caseId=${encodeURIComponent(caseId)}` });
  } catch { return NextResponse.json({ error: "Unable to securely upload the registration certificate." }, { status: 500 }); }
}

...
export async function POST(request, { params }) {
  ...
  try {
    ...
    if (!validation.ok) return NextResponse.json({ error: validation.error || "A valid PDF clearance report is required." }, { status: 400 });
    await storage.bucket().file(`portal-clients/${params.uid}/reports/${caseId}/clearance-report.pdf`).save(validation.buffer, { resumable: false, contentType: validation.contentType, metadata: { cacheControl: "private, no-store", metadata: { uploadedBy: staff.uid } } });
    return NextResponse.json({ success: true, documentUrl: `/api/portal/documents/report/${params.uid}?caseId=${encodeURIComponent(caseId)}` });
  } catch { return NextResponse.json({ error: "Unable to securely upload the clearance report." }, { status: 500 }); }
}

...
function invitationEmail({ name, email, markName, trademarks = [], setupUrl, source, returning = false }) {
  // A returning client whose password was never set still needs this email, so
  // avoid welcoming them as though the account had just been created.
  const intro = returning
    ? "Your payment was confirmed and the new trademark matter was added to your existing client portal. Your password has not been set yet, so use the secure link below to create it."
    : source === "checkout"
...
export async function provisionPortalClient({ name, email, company = "", phone = "", markName = "", markType = "Word mark", packageName = "", trademarks = [], transactionId = "", orderTotal = 0, source = "manual", applicationDetails = {}, billingProfile = {} }) {
  ...
  // Whether to send a setup link depends on whether this person can actually
  // sign in — not on whether their record is new. A client who paid before but
  // never finished creating a password would otherwise receive the "matter
  // added" email, whose only link is the login page they cannot get past.
  const hasPassword = (user.providerData || []).some(
    (provider) => provider.providerId === "password",
  );
  const needsPasswordSetup = newlyCreated || !hasPassword;
  const notification = {
    clientUid: user.uid,
    clientName: name,
    clientEmail: email,
  ...
  // Each SMTP round trip against the transactional host costs ~10s. Awaiting
  // the staff notice before the client email pushed the request past the
  // serverless timeout, so the browser saw a failure for work that had
  // actually succeeded. Start it here and settle it at the end instead.
  const attorneyNotice = sendAttorneyActivityEmail(notification).catch((error) => console.error("New client attorney email failed:", error?.message));
  ...
  if (!needsPasswordSetup) {
    ...
    await transporter.sendMail({ from: `Legal Trademark Office <${process.env.SUPPORT_EMAIL || process.env.MAILER_EMAIL}>`, to: email, subject: !isNewClient
        ? "Payment confirmed — set your client portal password"
        : source === "checkout"
          ? "Payment confirmed — your client portal is ready"
          : "Manage your trademark applications in your new client portal", html: invitationEmail({ name, email, markName, trademarks, setupUrl, source, returning: !isNewClient }) });
  }
  await attorneyNotice;

...
function cleanApplicationDetails(input = {}) {
  ...
  const entry = (item) => {
    if (item && typeof item === "object") {
      ...
      return label && Number.isFinite(classNo) && classNo > 0 ? `${label} (Class ${classNo})` : label;
    }
    return text(item, 180);
  };
  const list = (value) => (Array.isArray(value) ? value : []).map(entry).filter(Boolean).slice(0, 40);
  return {
...
export async function getClientPortalData(uid) {
  ...
  let messages;
  try {
    [client, cases, tasks, appointments, invoices, messages] = await Promise.all([
      clientRef.get(),

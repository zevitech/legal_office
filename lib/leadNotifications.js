...
import { recordLeadDelivery } from "@/lib/leadStore";
...
// Uploaded assets (sound marks, logos) are stored as Cloudinary URLs. Escaping
// them as plain text left staff with an unclickable string, so the audio file a
// customer uploaded appeared to be missing. Render them as links instead.
...
const renderCell = (value) => {
  if (isHttpUrl(value)) {
    ...
    return `<a href="${url}" target="_blank" rel="noopener noreferrer" style="color:#026daf;font-weight:700;text-decoration:underline;word-break:break-all">Open uploaded file</a><br><span style="color:#94a3b8;font-size:11px;word-break:break-all">${url}</span>`;
  }
...
const EMAIL_SECTIONS = [
  { title: "Trademark", keys: ["protectionTypes", "wantToProtect", "name", "slogan", "logo", "logoColors", "logoProtectionDescription", "soundDescription", "soundFileName", "soundFile", "trademarkCurrentlyBeingUsed", "firstAnywhereDate", "firstCommenceDate"] },
...
export async function deliverLeadNotifications(data, stored) {
      let emailSent = false;
      let emailError = "";
      let crmSent = false;
      let crmError = "";

...
Deployed 99a4d2b; Vercel DPLet1Liq2yucEoQjynuSyUaraEP confirmed Ready on the production domain. New genuine submission latency remains unmeasured.
...
## September 9: landing-page render regression follow-up

Supplied report `7p46nstjpt` (02:50 PDT): mobile 52, FCP 5.2s, LCP 10.2s, TBT 380ms, CLS 0; desktop 68, FCP 0.3s, LCP 0.8s, TBT 3400ms, CLS 0.017. Fresh live report `uc2u69h0at` (03:09 PDT, before this patch): mobile 66, FCP 3.1s, LCP 4.2s, TBT 520ms, CLS 0; desktop 53, FCP 0.3s, LCP 1.7s, TBT 7910ms, CLS 0.017. Both are lab runs with no real-user data. Prior 93/90 result is historical, not a stable guarantee.

The production HTML placed the entire landing page, including H1, in a hidden streamed segment behind its loading boundary. Extracted the exact hero JSX to a lightweight independent client island and put the remaining landing content behind its own Suspense boundary. The hero now appears in the initial visible HTML without waiting for below-fold dependencies or the stream-reveal script. All content, typography, layout classes, button text and destination URLs are retained. No GTM, Ads, Clarity, Reddit, LiveChat, payment, form, CAPTCHA or API implementation changed.

Rejected experiments: static package/FAQ imports and experimental UI-library import optimization did not improve local render timing, so both were reverted. No experimental configuration is retained.

Next 15.5.25 production build, lint/types and all 92 routes pass. Controlled local tests with 150ms network latency, 200kB/s download and 4x CPU slowdown measured mobile LCP 1864ms → 1072ms and desktop 1748ms → 1012ms. External scripts were blocked identically in BOTH local test builds to isolate first-party work and avoid sending test tracking events; no scripts were disabled in production source. These results are not a live Lighthouse score. Full content and computed hero styles match, no horizontal overflow, FAQ/review navigation and Business Plus selection into the form pass. Initial JS bytes are effectively unchanged (219792 → 219866); this is a rendering-order improvement, not a bundle-size claim.

Live reference: https://pagespeed.web.dev/analysis/https-www-legaltrademarkoffice-com-trademark-registration/uc2u69h0at
Release status: local verified candidate; production score after this patch not yet measured. Third-party main-thread work remains and 90+ is not guaranteed.
